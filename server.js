...
const app = express();
// Behind Application Gateway (a single reverse-proxy hop): trust its
// X-Forwarded-For so req.ip is the real client, not the ingress address.
// Without this every client shares one rate-limit bucket (see authRateLimiter).
...
// 🏥 Health check — wired to liveness/readiness/startup probes, so it must
// actually exercise the DB: otherwise Kubernetes routes traffic to a pod that
// can't serve a single request.
app.get('/api/health', async (req, res) => {
  try {
    await pool.query('SELECT 1');
    res.json({ status: 'ok', timestamp: new Date().toISOString() });
  } catch (err) {
    console.error('Health check failed: DB unreachable:', err?.message || err);
...
// DB Connection check
// ------------------------
// Schema no longer applies at boot — run `npm run migrate` (runMigration.js)
// against migrations/ instead, as its own step, so a broken migration fails
// that step loudly rather than being swallowed here while the app starts
// anyway. This is just a startup log; the pool connects lazily per query
// regardless, and /api/health is what actually gates traffic on DB health.
pool.connect()
  .then(client => {
...
// Fail closed like isZohoTicketWebhookAuthorized/isAlertmanagerAuthorized below:
// an unset key must reject every request, not admit them.
function validateMonitoringKey(req, res, next) {
  ...
  if (!safeTokenEqual(key, process.env.MONITORING_API_KEY)) {
    return res.status(401).json({ message: 'Unauthorized' });
  }
...
// ============================================================
app.post('/api/monitoring/intune/sync', authenticateToken, authorizeElevated, async (req, res) => {
  try {
    const token = await getGraphToken();
...
app.post('/api/monitoring/azure/seed', authenticateToken, authorizeElevated, async (req, res) => {
  try {
...
app.post('/api/monitoring/assets/seed', authenticateToken, authorizeElevated, async (req, res) => {
  try {
    const ownerEmail = (req.user?.email || '').toString().toLowerCase();
    if (!ownerEmail) return res.status(400).json({ message: 'No authenticated user email available' });
...
// 🔄 Refresh Token API
// Re-reads the role from the DB rather than trusting the refresh token's
// embedded claim — otherwise a role revoked via /api/admin stays valid in
// already-issued refresh tokens for up to their full 7-day life.
async function resolveStoredRoles(email) {
  ...
  try {
    ...
    const legacyRole = normalizeRole(legacy.rows[0]?.role);
    if (legacyRole) rolesSet.add(legacyRole);
  } catch (err) {
    console.warn(`[REFRESH] legacy role lookup failed for ${normalizedEmail}:`, err?.message || err);
  }
  ...
  try {
    ...
    for (const r of normalizeRoleList(bindings.rows[0]?.roles || [])) rolesSet.add(r);
  } catch (err) {
    console.warn(`[REFRESH] role bindings lookup failed for ${normalizedEmail}:`, err?.message || err);
  }

  return normalizeRoleList([...rolesSet], 'user');
}
...
app.post("/api/refresh", authRateLimiter, (req, res) => {
  ...
  jwt.verify(refreshToken, process.env.JWT_REFRESH_SECRET, async (err, user) => {
    ...
    const currentRoles = await resolveStoredRoles(user.email);
    const currentRole = currentRoles.includes(user.role) ? user.role : currentRoles[0];

    const newAccessToken = jwt.sign(
      { email: user.email, role: currentRole, roles: currentRoles },
      process.env.JWT_SECRET,
      { expiresIn: "1h" }
    );
...
app.post('/api/auth/switch-role', authenticateToken, async (req, res) => {
  try {
    ...
    // Re-sign the refresh token with the SAME expiry it already had, taken
    // from the client's current one, instead of a fresh 7 days — otherwise a
    // role switch resets the session clock and a session can be kept alive
    // indefinitely by switching roles before it would otherwise expire.
    let refreshExpiresInSeconds = null;
    const currentRefreshToken = req.body?.refreshToken;
    if (currentRefreshToken) {
      try {
        const decoded = jwt.verify(currentRefreshToken, process.env.JWT_REFRESH_SECRET);
        refreshExpiresInSeconds = Math.max(decoded.exp - Math.floor(Date.now() / 1000), 60);
      } catch (err) {
        console.warn('Role switch: current refresh token invalid, falling back to 7d:', err?.message || err);
      }
    }
    const refreshToken = jwt.sign(
      { email, role: requestedRole, roles: tokenRoles },
      process.env.JWT_REFRESH_SECRET,
      { expiresIn: refreshExpiresInSeconds || '7d' }
    );
...
app.delete('/api/tickets/recycle-bin/:ticketId', authenticateToken, authorizeElevated, async (req, res) => {
  try {
    const ticketId = (req.params.ticketId || '').toString();
    const userEmail = (req.user?.email || '').toLowerCase();
    const result = await pool.query(
...
app.post("/api/assignments", authenticateToken, async (req, res) => {
  try {
    ...
    // Actor comes from the authenticated token, not the request body, so the
    // audit trail can't be spoofed by whoever calls this endpoint.
    const assigned_by = (req.user?.email || '').toLowerCase();
...
app.put("/api/assignments/reassign", authenticateToken, async (req, res) => {
  try {
    const { zoho_ticket_id, new_assigned_users, category } = req.body;
    const reassigned_by = (req.user?.email || '').toLowerCase();
...
app.post("/api/assignments/bulk", authenticateToken, authorizeElevated, async (req, res) => {
  try {
    const { ticket_ids, assigned_users, ticket_categories, ticket_department_ids } = req.body;
    const assigned_by = (req.user?.email || '').toLowerCase();
...
app.put("/api/assignments/:zohoTicketId/close", authenticateToken, async (req, res) => {
  try {
    const closed_by = (req.user?.email || '').toLowerCase();
    ...
    const result = await pool.query(
      `UPDATE ticket_assignments SET
       ...
       WHERE zoho_ticket_id = $2
       RETURNING *`,
      [closed_by || null, req.params.zohoTicketId]
    );
...
// Per-process cache of the built report, avoiding a DB round trip on every
// poll. Persistence lives in Postgres (azure_backup_reports), not disk — the
// container filesystem has no volume and is wiped on every restart/rollout.
let latestAzureBackupReport = null;

async function fetchLatestAzureBackupRow() {
  const result = await pool.query(
    `SELECT filename, generated_at, records FROM azure_backup_reports ORDER BY generated_at DESC LIMIT 1`
  );
  return result.rows[0] || null;
}

async function readLatestAzureBackupReport() {
  if (latestAzureBackupReport) return latestAzureBackupReport;
  const row = await fetchLatestAzureBackupRow();
  if (!row) return null;
  const report = buildAzureBackupReport(row.records, row.filename, row.generated_at);
  latestAzureBackupReport = report;
  return report;
}

// Raw (PascalCase) records from the last saved report, used only as a fallback
// hint for vault discovery (azure-backup-collector.js) when the live Azure API
// call comes back empty. Distinct from readLatestAzureBackupReport(), which
// returns the normalized (camelCase) report shape for the UI.
async function fetchLatestRawAzureBackupRecords() {
  try {
    const row = await fetchLatestAzureBackupRow();
    return row?.records || [];
  } catch (err) {
    console.warn('[AZ-REPORT] Could not load known records from Postgres:', err?.message || err);
    return [];
  }
}
...
// NOT report.items. readLatestAzureBackupReport() re-parses this straight back through
// buildAzureBackupReport(), which expects that raw shape; saving the already-normalized
// (camelCase) report.items here would silently produce 0 records on the next read.
async function saveAzureBackupReport(records, filename, generatedAt) {
  const data = records || [];
  await pool.query(
    `INSERT INTO azure_backup_reports (filename, generated_at, records) VALUES ($1, $2, $3)`,
    [filename, generatedAt || new Date().toISOString(), JSON.stringify(data)]
  );
  console.log(`[AZ-REPORT] Saved report ${filename} to Postgres (${data.length} items)`);
}
  ...
  azureBackupJob = new Promise(async (resolve, reject) => {
    ...
    if (userToken) {
      try {
        ...
        const knownRecords = await fetchLatestRawAzureBackupRecords();
        let records = await collector.collectAzureBackupData(userToken, knownRecords);
        ...
        if (records && records.length > 0) {
          ...
          latestAzureBackupReport = report;
          await saveAzureBackupReport(records, filename, nowStr);
    ...
    if (triggerName !== 'scheduled' && process.env.AZURE_CLIENT_ID && process.env.AZURE_CLIENT_SECRET && process.env.AZURE_TENANT_ID) {
      try {
        const collector = require('./azure-backup-collector');
        const knownRecords = await fetchLatestRawAzureBackupRecords();
        let records = await collector.collectAzureBackupData(undefined, knownRecords);
        records = collector.deduplicateFileShares(records);
        if (records && records.length > 0) {
          const nowStr = new Date().toISOString();
          const filename = `AzureBackupData_${nowStr.replace(/[:.]/g, '-').slice(0, 19)}.json`;
...
async function checkAzureBackupSchedule() {
  if (String(process.env.ENABLE_AZURE_BACKUP_SCHEDULE || 'true').toLowerCase() === 'false') return;
  // Only run scheduled refresh if a previous report already exists (don't create empty reports)
  const latest = await readLatestAzureBackupReport();
  if (!latest) return;
...
app.listen(PORT, () => {
  ...
  // Poll for CloudOps ticket assignments, run the Azure Backup schedule check,
  // and sync presence — all single-instance jobs. Gated by ENABLE_ALERT_JOBS —
  // production runs this app as multiple replicas (see zoho-itsm Deployment)
  // plus one dedicated zoho-itsm-scheduler replica for single-instance jobs.
  // Running unconditionally here would run every job once per replica in
  // parallel, duplicating Teams notifications, Graph calls and DB writes.
  if (ENABLE_ALERT_JOBS) {
    checkAzureBackupSchedule().catch(err => console.error('[AZ-SCHEDULE] check failed:', err?.message || err));
    setInterval(() => checkAzureBackupSchedule().catch(err => console.error('[AZ-SCHEDULE] check failed:', err?.message || err)), 30 * 1000).unref();
...
app.get('/api/monitoring/azure/report-status', authenticateToken, authorizeElevated, async (req, res) => {
  // report-status is polled frequently (every 15-60s) — only pay the cost of a DB
  // round trip when we don't already have lastSuccess in memory.
  let fallbackGeneratedAt = null;
  if (!azureBackupLastSuccess) {
    try {
      const row = await fetchLatestAzureBackupRow();
      fallbackGeneratedAt = row ? new Date(row.generated_at).toISOString() : null;
    } catch (err) {
      console.warn('[AZ-STATUS] fallback lookup failed:', err?.message || err);
    }
  }
  res.json({
...
app.get('/api/monitoring/azure/report-log', authenticateToken, authorizeElevated, async (req, res) => {
  try {
    const structuredReport = await readLatestAzureBackupReport();
    if (structuredReport) {
      res.set('Cache-Control', 'no-store, max-age=0, must-revalidate');
...
// ------------------------
// Multer upload error handler — registered after every route, including the
// SPA fallback above. Express only walks FORWARD through the middleware stack
// looking for an error handler, so one placed earlier misses errors from any
